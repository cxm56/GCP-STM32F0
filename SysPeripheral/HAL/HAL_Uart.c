...
/**
  ...
  * @retval None
  */
static void HAL_UART_IOConfig(uint8_t uUartNode)
{
    switch (uUartNode)
    {
    case HAL_UART_NODE0:
    ...
    case HAL_UART_NODE1:
        // TX: PA2
        HAL_GPIO_ModeConfig(0, 2, GPIO_AF_PP_H);
        GPIOA->AFR[0]  &= ~(0XFU<<(2*4));
        GPIOA->AFR[0]  |=  (0X1U<<(2*4)); //USART_TX
        
        // RX: PA3
        HAL_GPIO_ModeConfig(0, 3, GPIO_AF_PP_H);
...
/**
  ...
  * @retval None
  */
static void HAL_UART_ConfigItCallBack(uint8_t uUartNode)
{
    //设置中断回调
    switch (uUartNode)
    {
...
/**
  ...
  * @retval None
  */
static void HAL_UART_DmaRxInit(uint8_t uUartNode)
{
    const uint8_t * const arBuffAddr[HAL_UART_NODE_NUM] = {Uart1_RecvBuff, Uart2_RecvBuff};
...
//UART1_Rx DMA通道复位
void HAL_UART_RxDMAReset(uint8_t uUartNode)
{
    //关闭通道
...
{
    //判断入参
    if (uUartNode >= HAL_UART_NODE_NUM)
    {
        return;
    }
    
    //设置中断回调
    HAL_UART_ConfigItCallBack(uUartNode);
    
    //初始化环形FIFO
    RingBuffer_Init(&m_RxRing[uUartNode], m_uRxBuff[uUartNode], 1, UART_RB_SIZE);
    
    //配置引脚
    HAL_UART_IOConfig(uUartNode);
    
    //配置工作模式
    HAL_UART_ModeConfig(uUartNode, ulBaudRate);
    
    //配置DMA
    HAL_UART_DmaRxInit(uUartNode);
...
void HAL_UART_SendBuff(uint8_t uUartNode, void *pSBuff, uint32_t ulSize)
{
    ...
    {
        HAL_UART_SendByte(uUartNode, ((uint8_t *)pSBuff)[i]);
    }
}
...
static void HAL_UART0_RX_IRQHandler(void)
{
    ...
    if (USART1->ISR & (0X1<<4))  //空闲帧中断入口
    {
        ...
        //复位DMA */
        HAL_UART_RxDMAReset(HAL_UART_NODE0);
...
static void HAL_UART1_RX_IRQHandler(void)
{
    ...
    if (USART2->ISR & (0X1<<4))  //空闲帧中断入口
    {
        ...
        //复位DMA
        HAL_UART_RxDMAReset(HAL_UART_NODE1);
