 ...
 * 私有成员定义及实现
 ****************************************************************************/
#define BK_EEPROM_STORE_ADDR        (64)    //EEPROM中数据存储的地址
...
uBit8 GC_SysParmInit(void)
{
    ...
    //从EEPROM中读取出存储的数据
    AT24Cxx_ReadMultiBytes(BK_EEPROM_STORE_ADDR, (uBit8 *)&m_GCSysParm, sizeof(GC_SYS_PARM));
    ...
    if (nCRC16 != m_GCSysParm.ulCRC16)
    {
        ...
        //存储数据
        AT24Cxx_WriteMultiBytes(BK_EEPROM_STORE_ADDR, (uBit8 *)&m_GCSysParm, sizeof(GC_SYS_PARM));
...
uBit8 GC_StoreSysParm(void)
{
    ...
    //回读EEPROM
    AT24Cxx_ReadMultiBytes(BK_EEPROM_STORE_ADDR, (uBit8 *)&TempSysParam, sizeof(GC_SYS_PARM));
